...
public class FileReceiver {
    ...
    public void getFiles() throws IOException{
        ...
        while(true){
            DatagramPacket data = new DatagramPacket(buffer2,buffer.length); //offset at which to start storing characters (0 in this case)
            socket.receive(data);

            byte[] aPacket = data.getData(); //a packet returned as an array of bytes, needs to be managedby
            PacketManager boss = new PacketManager(aPacket);
